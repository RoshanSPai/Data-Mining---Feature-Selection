function [accKnn,accLin,accCent,accSvm] = kfoldCV(trainData, classData, knn )
...
accKnn=mean(accuracyKNN)*100;
accLin=mean(accuracyCent)*100;
accCent=mean(accuracyLin)*100 ;
accSvm=mean(accuracyLibsvm);
end
